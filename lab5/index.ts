...
const findSmallestNumberAmongMixedElements = (arr: number[]) => {
    let numbers: number[] = arr.filter(el => Number.isInteger(el))
    if (numbers.length === 0) {
        return ""
    }

    return numbers.find((num1, i, arr) =>
        undefined === arr.find(num2 => num1 > num2))
}

const modulo = (number: number, divider: number) => {
    const res = number / divider
    return number - Math.trunc(res) * divider
}

const invert = (str: string, number: number = str.length) => {
...
const detectOutlierValue = (str: string) => {
...
const findPairForSum = (numbers: number[], sum: number) => {
...
const isReverted = (text: string, revertedText: string) => {
